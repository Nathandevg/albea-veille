...
def deduplicate(articles: list[Article]) -> list[Article]:
    ...
    Étape 2 : dédoublonnage par similarité de titre (> 90%).
    """
    if not articles:
        return []
    ...
    for article in articles:
        ...
        if url and url not in seen_urls:
            ...
            # Article sans URL, on le garde pour la suite
            unique_by_url.append(article)

    logger.info(f"After URL dedup: {len(unique_by_url)} (removed {len(articles) - len(unique_by_url)})")

    # Étape 2 : similarité de titre
    result: list[Article] = []
    ...
    logger.info(f"After title dedup: {len(result)} (removed {len(unique_by_url) - len(result)})")
